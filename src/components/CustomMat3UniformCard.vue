<template>
  <card-header :init-expanded="expanded" :title="$t('variable.custom.prefix') + 'mat3' + $t('variable.custom.suffix') + (uniformName.length === 0 ? '' : ': '+ uniformName)">
    ...
    <el-dialog v-model="showDeleteDialog" :title="$t('variable.custom.del.title')" width="30%" align-center>
      <span>{{
          ...
          $t('variable.custom.prefix') + 'mat3' + $t('variable.custom.suffix') + uniformName +
...
export default {
  ...
  props: {
    id: Number,
    parent: Object,
    initValue: {type: Object, default: {value: [1., 0., 0., 0., 1., 0., 0., 0., 1.], step: .1}},
  ...
  data() {
    return {
      ...
      value: (this.initValue.value || [1., 0., 0., 0., 1., 0., 0., 0., 1.]).concat(),
  ...
  methods: {
    ...
    genQuery() {
      return { type: 'mat3', initValue: { value: this.value, step: this.step }, name: this.name };
    }
  }
}
</script>
