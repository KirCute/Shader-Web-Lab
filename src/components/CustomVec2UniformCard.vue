<template>
  <card-header :init-expanded="expanded" :title="$t('variable.custom.prefix') + 'vec2' + $t('variable.custom.suffix') + (uniformName.length === 0 ? '' : ': '+ uniformName)">
    ...
    <el-dialog v-model="showDeleteDialog" :title="$t('variable.custom.del.title')" width="30%" align-center>
      <span>{{
          ...
          $t('variable.custom.prefix') + 'vec2' + $t('variable.custom.suffix') + uniformName +
