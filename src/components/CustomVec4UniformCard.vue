<template>
  <card-header :init-expanded="expanded" :title="$t('variable.custom.prefix') + 'vec4' + $t('variable.custom.suffix') + (uniformName.length === 0 ? '' : ': '+ uniformName)">
    <el-form label-width="100px" size="small">
      ...
      <el-form-item :label="$t('variable.custom.value')">
        ...
        <el-input-number v-model="value[3]" :step="step" class="inline"/>
    ...
    <el-dialog v-model="showDeleteDialog" :title="$t('variable.custom.del.title')" width="30%" align-center>
      <span>{{
          ...
          $t('variable.custom.prefix') + 'vec4' + $t('variable.custom.suffix') + uniformName +
...
export default {
  ...
  computed: {
    color: {
      get: function () {
        return rgbaToHex(this.value[0], this.value[1], this.value[2], this.value[3]);
      },
  ...
  methods: {
    ...
    genQuery() {
      return { type: 'vec4', initValue: { value: this.value, step: this.step }, name: this.name };
    }
  }
}
</script>
