<template>
  <card-header :init-expanded="expanded" :title="$t('variable.custom.prefix') + 'mat2' + $t('variable.custom.suffix') + (uniformName.length === 0 ? '' : ': '+ uniformName)">
    <el-form label-width="100px" size="small">
      ...
      <el-form-item>
        <el-input-number v-model="value[2]" :step="step" class="inline-first"/>
    ...
    <el-dialog v-model="showDeleteDialog" :title="$t('variable.custom.del.title')" width="30%" align-center>
      <span>{{
          ...
          $t('variable.custom.prefix') + 'mat2' + $t('variable.custom.suffix') + uniformName +
...
export default {
  name: "CustomMat2UniformCard",
  props: {
    id: Number,
    parent: Object,
    initValue: {type: Object, default: {value: [1., 0., 0., 1.], step: .1}},
  ...
  data() {
    return {
      ...
      value: (this.initValue.value || [1., 0., 0., 1.]).concat(),
  ...
  methods: {
    ...
    bindUniform(gl, shaderProgram) {
      ...
      gl.uniformMatrix2fv(uni, true, this.value);
    },
    genQuery() {
      return { type: 'mat2', initValue: { value: this.value, step: this.step }, name: this.name };
    }
  }
}
</script>
